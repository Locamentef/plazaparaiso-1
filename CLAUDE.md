...
6. **Enlaces internos** — el artículo enlaza a las landings de los eventos que menciona y a
   `festival.html` / `programa.html` / `entradas.html` y a otros artículos. **Todo enlace interno a una página del
   blog (listado o artículo) se escribe en URL limpia raíz que coincide EXACTAMENTE con el canonical: `/blog` y
   `/blog/<slug>`, sin `.html`, sin `/index.html` y sin barra final.** Además, "Blog" debe estar
...
- **Rutas relativas con `../../`**: al estar dentro de `blog/<slug>/`, todos los recursos de la raíz se
  ...
  **Enlaces a páginas del blog en URL limpia raíz** (coinciden con el canonical): el listado es `/blog` y los
  enlaces a OTROS artículos son `/blog/<otro-slug>` (sin `.html`, sin `/index.html`, sin barra final). El resto de
  recursos (CSS, imágenes, otras páginas) siguen con `../../`.
- Reescribir head (title, description, keywords, canonical, og/twitter, JSON-LD `BlogPosting` + breadcrumb) y el `<main>`.
- Añadir una tarjeta en el grid de `blog.html` (enlace `/blog/<slug>`) y un `BlogPosting` al schema `Blog` de `blog.html`.
...
- Las URLs limpias (`/blog/<slug>`, `/festival`, etc.) las resuelve Netlify (Pretty URLs) + los `canonical`.
  Los enlaces internos a páginas del blog se escriben en esa forma limpia raíz (`/blog`, `/blog/<slug>`) para
  coincidir EXACTAMENTE con el canonical (sin `.html` ni barra final). Nota: en el preview local esos enlaces de
  blog no resuelven al archivo (sí funcionan ya desplegados en Netlify); los recursos (CSS/imágenes) van con `../../`.
